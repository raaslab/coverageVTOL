...
clear
close all
load /home/user01/Kevin_Yu/coverageVTOL/VTOL/outputs/qualitativeExamples/journalQg1.mat
load /home/user01/Kevin_Yu/coverageVTOL/VTOL/outputs/qualitativeExamples/journalQg3.mat
...
[sMulti, sFixed, sGround, tMulti,tFixed,tGround] = deal([]);
impossible = 0;
lastPoint = 0;
...
for a = 2:(numPointsInit/2)+1
    ...
    if typeChecker == 1 || typeChecker == 7 || typeChecker == 8 || typeChecker == 9 % MM
        ...
        sMulti(end+1) = GLNSSolutionWithAllPoints(locationStart);
        tMulti(end+1) = GLNSSolutionWithAllPoints(locationEnd-1);
        if lastPoint == 0
            S3(end+1) = GLNSSolutionWithAllPoints(locationStart+1);
            T3(end+1) = GLNSSolutionWithAllPoints(locationEnd);
            sMulti(end+1) = GLNSSolutionWithAllPoints(locationStart+1);
            tMulti(end+1) = GLNSSolutionWithAllPoints(locationEnd);
        end
    elseif typeChecker == 2 % MG
        ...
        if lastPoint == 0
            ...
            sGround(end+1) = GLNSSolutionWithAllPoints(locationStart+1);
            tGround(end+1) = GLNSSolutionWithAllPoints(locationEnd);
        end
    elseif typeChecker == 3 || typeChecker == 13 || typeChecker == 14 || typeChecker == 15 % MF
        ...
        if lastPoint == 0
            ...
            sFixed(end+1) = GLNSSolutionWithAllPoints(locationStart+1);
            tFixed(end+1) = GLNSSolutionWithAllPoints(locationEnd);
        end
    elseif typeChecker == 4 || typeChecker == 16 || typeChecker == 17 || typeChecker == 18 % FM
        ...
        sFixed(end+1) = GLNSSolutionWithAllPoints(locationStart);
        tFixed(end+1) = GLNSSolutionWithAllPoints(locationEnd-1);
        if lastPoint == 0
...
% CREATING THE PLOT WITH SPLIT EDGES GRAPH2
if impossible == 0
    figure(3)
    GLNSPlot = plot(GLNSg,'XData',GLNSx,'YData',GLNSy, 'LineWidth',3, 'EdgeColor', [0,0,1]);
    GLNSPlot.EdgeAlpha = 1;
    ...
    if isempty(sMulti) == 0                 %highlight type 1 edges: multirotor
        highlight(GLNSPlot,sMulti, tMulti,'EdgeColor',[230,25,75]/255,'LineWidth',4, 'LineStyle', '-')
    end
    if isempty(sFixed) == 0                 %highlight type 2 edges: fixed-wing
        highlight(GLNSPlot,sFixed, tFixed,'EdgeColor',[60,180,75]/255,'LineWidth',3, 'LineStyle', '-')
    end
    if isempty(sGround) == 0                 %highlight type 3 edges: ground vehicle
        highlight(GLNSPlot,sGround, tGround,'EdgeColor',[0,130,200]/255,'LineWidth',3, 'LineStyle', '-')
    end
    if isempty(downUpPoints) == 0
        highlight(GLNSPlot,downUpPoints,'MarkerSize',10)
        highlight(GLNSPlot,downUpPoints,'NodeColor',[0,0,0]/255)
...
h = zeros(3, 1);
h(1) = plot(NaN,NaN,'color', [230,25,75]/255);
h(2) = plot(NaN,NaN,'color', [60,180,75]/255);
