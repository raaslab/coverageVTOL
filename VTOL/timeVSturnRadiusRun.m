...
loop = 0;
for trial  = 1:10
    for turnRadius = 1:1:10
        
        filename4 = sprintf('inputs/timeVSturnRadius/%d_%d.txt',trial,turnRadius);
        ...
        filename1 = sprintf('%d_%d_1',trial,turnRadius);
        filename2 = sprintf('/home/user01/Kevin_Yu/3D_bridge_meshes/coverage/VTOL/outputs/timeVSturnRadius/%d_%d_2.gtsp', trial,turnRadius);
        filename3 = sprintf('%d_%d_3',trial,turnRadius);
        
        tic
        pathName = '/home/user01/Kevin_Yu/3D_bridge_meshes/coverage/VTOL/outputs/timeVSturnRadius';
        ...
        trialTime = toc
        timeTurnRadius(end+1, :) = [trial, double(turnRadius), trialTime];
        fprintf("loop:%d\n",loop)
        loop = loop+1;
    end
end
