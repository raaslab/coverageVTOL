...
function [] = polygonCreaterSmall(fileName,numberOfPolygons,randNum,showFigs,TSPReplica,belowNum)
...
while i < numberOfPolygons+1
    ...
    lengthCheck = sqrt((x1(i)-x2(i))^2 + (y1(i)-y2(i))^2);
    if lengthCheck >= belowNum
        repeat = 1;
        skipInterCheck = 1;
    end
    
    if i ~= 1 || skipInterCheck == 0
        for j = 1:i-1
