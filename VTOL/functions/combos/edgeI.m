...
function [outputEdges] = edgeI(F, DTU, v_Cluster, distances, levels, sites, groupedPoints)
    ...
    while j < (sites*levels)+1
        if v_Cluster(i) == v_Cluster(j)
            ...
            if max(F(i,tempLocations) ~= Inf) == 1
                ...
                tempPossiblePath = distances(groupedPoints(i),groupedPoints(middlePoint)) + DTU(middlePoint,j); % F + DTU
