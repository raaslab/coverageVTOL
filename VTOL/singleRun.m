...
UGVS = 100;          % time to travel one unit for the UGV to multi-rotor (greater than 1 means UGV is slower)
method = 1;        % 1 = GLNS, 0 = concorde
timeI = [];
fixedRatio = 6;     % ratio of multi-rotor to fixed-wing battery usage per distance (1lvl/5m : 1lvl/10m = 2) (greater than 1 means fixed wing goes farther on one battery level)
turnRadius = 0.5;     % turn radius for dubins constraints in 
...
filename1 = sprintf('journalQi1');
filename2 = sprintf('/home/user01/Kevin_Yu/3D_bridge_meshes/coverage/VTOL/outputs/journalQi2.gtsp');
filename3 = sprintf('journalQi3');
