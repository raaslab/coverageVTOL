...
loop = 0;
for trial  = 1:10
    for fixedRatio = 1:1:20
        
        filename4 = sprintf('inputs/timeVSfixedRatio/%d_%d.txt',trial,fixedRatio);
        ...
        filename1 = sprintf('%d_%d_1',trial,fixedRatio);
        filename2 = sprintf('/home/user01/Kevin_Yu/3D_bridge_meshes/coverage/VTOL/outputs/timeVSfixedRatio/%d_%d_2.gtsp', trial,fixedRatio);
        filename3 = sprintf('%d_%d_3',trial,fixedRatio);
        
        tic
        pathName = '/home/user01/Kevin_Yu/3D_bridge_meshes/coverage/VTOL/outputs/timeVSfixedRatio';
        ...
        save(f);
        trialTime = toc
        timeFixedRatio(end+1, :) = [trial, double(fixedRatio), trialTime];
        fprintf("loop:%d\n",loop)
        loop = loop + 1;
    end
end

save('timeVSfixedRatio.mat')
